...
public class ChannelTextRepositoryThread extends Thread {
    ...
    public ChannelTextRepositoryThread(BlockingQueue<ChatMessage> repositoryBoundChatMessagesQueue,
            MarkovChainRepository repository) {
    ...
    /**
     * convert single string to markov chain, and then adds them to the repo. We use database constraints to prevent
     * duplicates from being read by the database. Since having a duplicate is expected, we catch the exceptions and
     * continue forth.
     * 
     * @param str
     */
